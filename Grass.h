...
{
	float GenRandomRumber(int min, int max);
	float GenRandomRumber_s(int min, int max);
	...
	const GLchar *GeometryShaderSourceCode_q =
		...
		"			windPowerDeflection = 20.0*sin(patchPosition.x/30.0+patchPosition.z/30+u_time*(0.2+windStrength/10.0));	\n	" \
		"			windPower = 1.5 + (windPowerDeflection*2.0f);													\n	" \
		"			if(windPower < 0.0)																		\n	" \
		...
		"		temp_tr_n = normalize(vec3(0.0, 1.0, 0.0));									\n	" \
		...
		"		temp_tl_n = normalize(vec3(0.0, 1.0, 0.0));												\n	" \
	...
	const GLchar *FragmentShaderSourceCode_q =
		...
		"			else														\n	" \
		"			{															\n	" \
		"				finalColor = vec4(texColor.xyz * u_yellow_color * 1.0, 1.0);	\n	" \
		"			}															\n	" \
		"																		\n	" \
	...
	//LOCATION CALCULATION LOGIC FOR GRASS AND SUNFLOWER
	
	//glm::vec3 grassLocartions[4455];
	//srand(time(0));
	////fill array
	//for (int i = 0; i< 1000; i++)
	//{
	//	grassLocartions[i].x = GenRandomRumber(1, 1200);
	//	grassLocartions[i].z = GenRandomRumber(1, 1200);
	//}
	//int c = 10;
	//int count = 1000;
	//fprintf(gpfile, " grass locations scene 1\n\n");
	//for (size_t i = 0; i < 1000; i++)
	//{
	//	grassLocartions[i].x *= -1.0f;
	//	grassLocartions[i].z *= -1.0f;
	//	grassLocartions[i].y = pTerrainInst->GetHeightAt(grassLocartions[i]);
	//	if (grassLocartions[i].y > 50.0f) 
	//	{
	//		--count;
	//		grassLocartions[i].x = 0.0;
	//		grassLocartions[i].y = 0.0;
	//		grassLocartions[i].z = 0.0;
	//	}
	//	else
	//	{
	//		float value = (float)(rand() % 100) / 100.0f;
	//		fprintf(gpfile, "%f, ", value);
	//		c--;
	//		if (c == 0)
	//		{
	//			c = 10;
	//				fprintf(gpfile, "\n");
	//		}
	//		//fprintf(gpfile, "glm::vec3(%f, %f, %f), \n", grassLocartions[i].x, grassLocartions[i].y, grassLocartions[i].z);
	//	}
	//}
	//fprintf(gpfile, "****count of grass finally: %d\n", count);
	// origin
	//grassLocartions[0].x = 0.0f;
	//grassLocartions[0].z = 0.0f;
	//grassLocartions[0].y = pTerrainInst->GetHeightAt(grassLocartions[0]);
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(grassLocation_All), &grassLocation_All[0], GL_STATIC_DRAW);
...
void displayGrass(glm::mat4 Model, glm::mat4 View, glm::mat4 Projection, CSunLight *SunLight)
{
	//glm::mat4 model;
	//glm::mat4 view;
	//glm::mat4 projection;
	...
	if (gbToggleLights == true)	// lights are on
	{
		...
		glUniform3fv(ld_q, 1, glm::value_ptr(SunLight->getDiffuse()));
		glUniform3fv(kd_q, 1, materialDiffuse);
		glUniform4fv(lightPosition_q, 1, glm::value_ptr(SunLight->getDirection()));
	}
	else						// lights are off
	{
...
float GenRandomRumber(int min , int max)
{
	if (max == 0)
		max = 1;
	//float r = ((float)rand() / (float)(max+min+1)) + (float)min;
	float r = ((float)rand() / (float)RAND_MAX * max) + (float)min;
	return r;
}
